...
algoParameters = [{'low': 2, 'high': 200}, {'low': 0.001, 'high': 100}]
...
def ada_boost(a, b, c, d, candidate):
    if candidate['tunings'][1] <= 0:
        candidate['tunings'][1] = random.choice(learning_rate)

    model = AdaBoostClassifier(n_estimators=candidate['tunings'][0], learning_rate=candidate['tunings'][1],
                               algorithm=candidate['tunings'][2])
    model.fit(a, b)
...
def score(candidate, datasets):
    ...
    r = ada_boost(train_X, train_y, test_X, test_y, candidate)
    r = round(r, 2)
    return r
...
def extrapolate(np, old, pop, cr, f, noOfParameters, index, dataset):
    ...
    for i in range(0, noOfParameters):
        ...
        elif old['tunings'][i] in algorithm:
            newf.append(random.choice(algorithm))
